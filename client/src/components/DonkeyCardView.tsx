...
export const DonkeyCardView: React.FC<DonkeyCardViewProps> = ({
  ...
  if (isCompact) {
    return (
      ...
      >
        {!isValid && <div className="absolute inset-0 bg-slate-950/15 pointer-events-none z-20" />}
  ...
  return (
    <div
      ...
      className={`relative w-16 h-24 sm:w-20 sm:h-28 rounded-2xl bg-white border border-slate-200/90 p-2 flex flex-col justify-between select-none transition-all duration-150 cursor-pointer overflow-hidden ${
        ...
        isValid
          ? 'hover:-translate-y-2.5 hover:scale-102 hover:shadow-[0_14px_28px_rgba(0,0,0,0.5)] active:scale-95'
          : 'brightness-[0.84] opacity-85 hover:-translate-y-0.5 active:scale-98'
      }`}
    >
      {!isValid && <div className="absolute inset-0 bg-slate-950/15 pointer-events-none z-20" />}
